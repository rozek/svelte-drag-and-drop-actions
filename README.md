...
* **`PanSpeed?:number`**<br>&nbsp;<br>`PanSensorHeight` is an optional ordinal number (defaulting to `10`) which specifies the "speed" of automatic scrolling - values in the range of 10...20 are reasonable choices, but it is always a good idea to make this parameter configurable for the users of your application. If set to `0`, no panning will be performed
